...
export const getHubs = asyncHandler(async (req, res) => {
  ...
  const hubsWithZones = await Promise.all(
    hubs.map(async (hub) => {
      ...
      return {
        ...hub,
        assignedZones: hubZones
          .map((hubZone) => ({
            _id: hubZone.zoneId?._id,
            name:
              hubZone.zoneId?.name ||
              hubZone.zoneId?.zoneName ||
              hubZone.zoneId?.serviceLocation,
          }))
...
export const createHub = asyncHandler(async (req, res) => {
  ...
  if (!hubName || !managerName || !email || !password) {
    return errorResponse(
      res,
      400,
      "Hub name, manager name, email, and password are required",
    );
  }
  ...
  const validZoneIds = zoneIds.filter((id) =>
