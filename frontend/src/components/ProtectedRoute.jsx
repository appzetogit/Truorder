...
export default function ProtectedRoute({ children, requiredRole, loginPath }) {
  ...
  const isDeliveryRoute = requiredRole === "delivery";
  const isOnboardingRoute = location.pathname.startsWith("/restaurant/onboarding");
  const isDeliverySignupDetailsRoute =
    location.pathname.startsWith("/delivery/signup/details");
  const isDeliverySignupDocumentsRoute =
    location.pathname.startsWith("/delivery/signup/documents");
  const isDeliverySignupRoute =
    isDeliverySignupDetailsRoute || isDeliverySignupDocumentsRoute;
  const [verificationState, setVerificationState] = useState({
    checked: false,
    isActive: true,
    rejectionReason: "",
  });
  const [deliverySignupState, setDeliverySignupState] = useState({
    checked: false,
    needsSignup: false,
    redirectPath: "/delivery/signup/details",
  });
  ...
  useEffect(() => {
    ...
    const hasMissingBasicProfile = (delivery) =>
      !delivery?.name?.trim() ||
    ...
    const hasMissingDocuments = (delivery) =>
      !delivery?.documents?.aadhar?.document ||
      !delivery?.documents?.pan?.document ||
      !delivery?.documents?.drivingLicense?.document;

    const loadDeliverySignupState = async () => {
      try {
        const response = await deliveryAPI.getCurrentDelivery();
        const delivery = response?.data?.data?.user;
        if (!isMounted || !delivery) return;

        const requiresFullProfileStatus =
          !["approved", "active"].includes(delivery.status);
        const missingBasicProfile = hasMissingBasicProfile(delivery);
        const missingDocuments = hasMissingDocuments(delivery);
        const needsSignup =
          requiresFullProfileStatus &&
          (missingBasicProfile || missingDocuments);

        let redirectPath = "/delivery/signup/details";
        if (!missingBasicProfile && missingDocuments) {
          redirectPath = "/delivery/signup/documents";
        }

        try {
          localStorage.setItem("delivery_user", JSON.stringify(delivery));
        } catch {
          // ignore storage failures
        }

        setDeliverySignupState({
          checked: true,
          needsSignup,
          redirectPath,
        });
        ...
        if (!isMounted) return;
        setDeliverySignupState((prev) => ({ ...prev, checked: true }));
      }
    };

    loadDeliverySignupState();
    return () => {
      isMounted = false;
    };
  }, [isDeliveryRoute]);
  ...
  if (isDeliveryRoute && !deliverySignupState.checked) {
    return null;
  }

  if (isDeliveryRoute && deliverySignupState.needsSignup) {
    if (
      isDeliverySignupRoute &&
      location.pathname === deliverySignupState.redirectPath
    ) {
      return children;
    }
    ...
    return (
      <Navigate
        to={deliverySignupState.redirectPath}
